...
layui.use('table', function(){
    ...
    tableIns = table.render({
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'tag_name', title: '标签名',width:150}
            ,{field: 'description', title: '标签描述',width:200}
    ...
    function renderData() {
        //var tag_name=$.trim($('input[name="tag_name"]').val());
        var where={};
        /*if(username!=''){
    ...
    function iniAddEditDataCtn(type,oldData) {
        ...
        if (type=='add'){
            ...
            var url=gaddMemberTagUrl;
        }else if(type == 'edit'){
            ctn.removeClass('add-data');
            ctn.addClass('edit-data');
            var url=saveMemberTagUrl;
            data.tag_id=oldData.id;
        }

        ctn.find('#dataSubmit').unbind('click').on('click',function () {
            var tagName=ctn.find('input').val();
        ...
        ctn.find('input[name="tag_name"]').val(oldData.tag_name);
