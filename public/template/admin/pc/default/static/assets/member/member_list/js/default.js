...
layui.use('table', function(){
    ...
    tableIns = table.render({
        ...
        ,cols: [[ //表头
            ...
            ,{field: 'group_name', title: '用户组', width:150}
            ,{field: 'tag_name', title: '标签', width:350,templet:function (d) {
                    var html='';
                    if (d.tag_name){
                        var nameStr=d.tag_name
                        var nameArr=nameStr.split(',');

                        for (var i in nameArr){
                            html+='<span class="layui-badge layui-bg-orange tag-style">'+nameArr[i]+'</span>'
                        }
                    }
                    return html;
                }}
            ...
            ,{width:130, title: '登陆日志',width:130,align:'center',event:'loginLog',hide:true,style:'color: #009688;cursor:pointer',templet:function () {
    ...
    table.on('tool(table1)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        switch(layEvent){
            ...
            case 'edit': //编辑行
                layer.open({
                    type:2,
                    title:'正在修改 '+data.phone+' 的信息',
                    area:['600px','500px'],
                    content:editMemberPageUrl+'?id='+data.id,
                });
                break;
            case 'status':
                alert(1);
